...
namespace FastCsv.Mapping.Converters;
...
public class EnumConverter : ICsvConverter
{
    ...
    public object? ConvertFromString(string value, Type targetType, string? format = null)
    {
            ...
            try
            {
                ...
                foreach (var part in parts)
                {
                    ...
                    try
                    {
                        ...
                        result |= Convert.ToInt32(partValue);
                    }
                    catch (ArgumentException)
                    {
        ...
        throw new FormatException($"Unable to parse '{value}' as {enumType.Name}. Valid values are: {validValues}");
    }
