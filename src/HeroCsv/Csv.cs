...
public static partial class Csv
{
    ...
    public static IEnumerable<T> Read<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.PublicConstructors)] T>(string content) where T : class, new()
    {
    ...
    [RequiresUnreferencedCode("Generic CSV mapping uses reflection to map data to objects. The type T must be preserved.")]
    [RequiresDynamicCode("Generic CSV mapping may create instances of types at runtime.")]
    public static IEnumerable<T> Read<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.PublicConstructors)] T>(string content, CsvOptions options) where T : class, new()
    {
    ...
    [RequiresUnreferencedCode("Generic CSV mapping uses reflection to map data to objects. The type T must be preserved.")]
    [RequiresDynamicCode("Generic CSV mapping may create instances of types at runtime.")]
    public static IEnumerable<T> Read<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.PublicConstructors)] T>(string content, CsvMapping<T> mapping) where T : class, new()
    {
    ...
    [RequiresUnreferencedCode("Generic CSV mapping uses reflection to map data to objects. The type T must be preserved.")]
    [RequiresDynamicCode("Generic CSV mapping may create instances of types at runtime.")]
    public static IEnumerable<T> Read<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.PublicConstructors)] T>(string content, Func<CsvMappingBuilder<T>, CsvMapping<T>> buildMapping) where T : class, new()
    {
    ...
    [RequiresUnreferencedCode("Generic CSV mapping uses reflection to map data to objects. The type T must be preserved.")]
    [RequiresDynamicCode("Generic CSV mapping may create instances of types at runtime.")]
    public static IEnumerable<T> ReadFile<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.PublicConstructors)] T>(string filePath) where T : class, new()
    {
    ...
    [RequiresUnreferencedCode("Generic CSV mapping uses reflection to map data to objects. The type T must be preserved.")]
    [RequiresDynamicCode("Generic CSV mapping may create instances of types at runtime.")]
    public static IEnumerable<T> ReadFile<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.PublicConstructors)] T>(string filePath, CsvOptions options) where T : class, new()
    {
        var content = File.ReadAllText(filePath);
    ...
    [RequiresUnreferencedCode("Generic CSV mapping uses reflection to map data to objects. The type T must be preserved.")]
    [RequiresDynamicCode("Generic CSV mapping may create instances of types at runtime.")]
    public static IEnumerable<T> ReadFile<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.PublicConstructors)] T>(string filePath, CsvMapping<T> mapping) where T : class, new()
    {
        var content = File.ReadAllText(filePath);
    ...
    [RequiresUnreferencedCode("Generic CSV mapping uses reflection to map data to objects. The type T must be preserved.")]
    [RequiresDynamicCode("Generic CSV mapping may create instances of types at runtime.")]
    public static IEnumerable<T> ReadFile<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.PublicConstructors)] T>(string filePath, Func<CsvMappingBuilder<T>, CsvMapping<T>> buildMapping) where T : class, new()
    {
        var content = File.ReadAllText(filePath);
        return Read<T>(content, buildMapping);
    }

    // ==================== AOT-Safe Mapping Methods ====================
    // These methods use factory functions instead of reflection for full AOT compatibility

    /// <summary>
    /// Reads CSV content and maps each record using a factory function (AOT-safe, no reflection)
    /// </summary>
    /// <typeparam name="T">Type to create</typeparam>
    /// <param name="content">CSV content to parse</param>
    /// <param name="factory">Factory function to create objects from CSV records</param>
    /// <returns>Enumerable of mapped objects</returns>
    /// <example>
    /// var people = Csv.Read(csvData, record => new Person
    /// {
    ///     Name = record.GetString(0),
    ///     Age = record.GetInt32(1)
    /// });
    /// </example>
    public static IEnumerable<T> Read<T>(string content, Func<ICsvRecord, T> factory)
    {
        return Read(content, CsvOptions.Default, factory);
    }

    /// <summary>
    /// Reads CSV content with custom options and maps using a factory function (AOT-safe)
    /// </summary>
    /// <typeparam name="T">Type to create</typeparam>
    /// <param name="content">CSV content to parse</param>
    /// <param name="options">Parsing options</param>
    /// <param name="factory">Factory function to create objects from CSV records</param>
    /// <returns>Enumerable of mapped objects</returns>
    public static IEnumerable<T> Read<T>(string content, CsvOptions options, Func<ICsvRecord, T> factory)
    {
        using var reader = CreateReader(content, options);
    ...
    /// <summary>
    /// Reads CSV content with headers and maps using a factory function that receives headers (AOT-safe)
    /// </summary>
    /// <typeparam name="T">Type to create</typeparam>
    /// <param name="content">CSV content to parse</param>
    /// <param name="factory">Factory function that receives headers and record</param>
    /// <returns>Enumerable of mapped objects</returns>
    /// <example>
    /// var people = Csv.ReadWithHeaders(csvData, (headers, record) => new Person
    /// {
    ///     Name = record.GetString(Array.IndexOf(headers, "Name")),
    ///     Age = record.GetInt32(Array.IndexOf(headers, "Age"))
    /// });
    /// </example>
    public static IEnumerable<T> ReadWithHeaders<T>(string content, Func<string[], ICsvRecord, T> factory)
    {
        var options = new CsvOptions(hasHeader: true);
        return ReadWithHeaders(content, options, factory);
    }

    /// <summary>
    /// Reads CSV content with headers and custom options, maps using a factory function (AOT-safe)
    /// </summary>
    /// <typeparam name="T">Type to create</typeparam>
    /// <param name="content">CSV content to parse</param>
    /// <param name="options">Parsing options</param>
    /// <param name="factory">Factory function that receives headers and record</param>
    /// <returns>Enumerable of mapped objects</returns>
    public static IEnumerable<T> ReadWithHeaders<T>(string content, CsvOptions options, Func<string[], ICsvRecord, T> factory)
    {
        using var reader = CreateReader(content, options);
        ...
        {
            throw new InvalidOperationException("CSV does not contain headers");
        }

        var headers = ExtensionsToICsvRecord.ToArray(headerRecord);
        var results = new List<T>();

        while (reader.TryReadRecord(out var record))
        {
            results.Add(factory(headers, record));
        }
    ...
    /// <summary>
    /// Reads CSV file and maps using a factory function (AOT-safe)
    /// </summary>
    /// <typeparam name="T">Type to create</typeparam>
    /// <param name="filePath">Path to CSV file</param>
    /// <param name="factory">Factory function to create objects from CSV records</param>
    /// <returns>Enumerable of mapped objects</returns>
    public static IEnumerable<T> ReadFile<T>(string filePath, Func<ICsvRecord, T> factory)
    {
        var content = File.ReadAllText(filePath);
        return Read(content, factory);
    }

    /// <summary>
    /// Reads CSV file with custom options and maps using a factory function (AOT-safe)
    /// </summary>
    /// <typeparam name="T">Type to create</typeparam>
    /// <param name="filePath">Path to CSV file</param>
    /// <param name="options">Parsing options</param>
    /// <param name="factory">Factory function to create objects from CSV records</param>
    /// <returns>Enumerable of mapped objects</returns>
    public static IEnumerable<T> ReadFile<T>(string filePath, CsvOptions options, Func<ICsvRecord, T> factory)
    {
        var content = File.ReadAllText(filePath);
        return Read(content, options, factory);
    }
    ...
    [RequiresUnreferencedCode("Generic CSV mapping uses reflection to map data to objects. The type T must be preserved.")]
    [RequiresDynamicCode("Generic CSV mapping may create instances of types at runtime.")]
    public static IEnumerable<T> ReadAutoMapWithOverrides<
    ...
    >(string content, CsvOptions options, Action<CsvMapping<T>> configureMapping) where T : class, new()
    {
    ...
    [RequiresUnreferencedCode("Generic CSV mapping uses reflection to map data to objects. The type T must be preserved.")]
    [RequiresDynamicCode("Generic CSV mapping may create instances of types at runtime.")]
    public static IEnumerable<T> ReadStream<
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.PublicProperties)] T
    >(Stream stream, CsvOptions options, bool leaveOpen = false) where T : class, new()
    {
        using var reader = CreateReader(stream, options, leaveOpen: leaveOpen);
    ...
    public static IEnumerable<T> ReadStream<
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.PublicProperties)] T
    >(Stream stream, CsvMapping<T> mapping) where T : class, new()
    {
    ...
    [RequiresUnreferencedCode("Generic CSV mapping uses reflection to map data to objects. The type T must be preserved.")]
    [RequiresDynamicCode("Generic CSV mapping may create instances of types at runtime.")]
    private static IEnumerable<T> ReadWithMapper<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.PublicConstructors)] T>(string content, CsvOptions options, CsvMapper<T> mapper) where T : class, new()
    {
        using var reader = CreateReader(content, options);
