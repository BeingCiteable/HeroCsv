...
public static partial class CsvParser
{
    ...
    public static string[] ParseLine(ReadOnlySpan<char> line, CsvOptions options)
    {
        ...
        // Use Vector512 if available and line is suitable
        if (Vector512.IsHardwareAccelerated && line.Length >= 64 && line.IndexOf('"') < 0 && options.StringPool != null)
        {
            var result = ParseLineVector512(line, options.Delimiter, options.StringPool);
            return [.. result];
        }
#endif

#if NET8_0_OR_GREATER
        // Use enhanced SearchValues and SIMD for .NET 8+
        if (Avx2.IsSupported && line.Length >= 32 && line.IndexOf('"') < 0)
        {
            return ParseSimpleCommaLineSIMD(line, options.StringPool);
        }
#endif
    ...
    private static unsafe string[] ParseSimpleCommaLineSIMD(ReadOnlySpan<char> line, StringPool? stringPool = null)
    {
                ...
                if (matchBitmask != 0)
                {
                    for (int bitPosition = 0; bitPosition < 32; bitPosition += BYTES_PER_CHAR)
                    {
                        if ((matchBitmask & (1u << bitPosition)) != 0)
                        {
                            ...
                            {
                                fieldArray[currentFieldIndex++] = CreateString(line[fieldStartPosition..commaCharPosition], stringPool);
        ...
        while (currentPosition < line.Length)
        {
            if (line[currentPosition] == COMMA_SEPARATOR)
            {
                fieldArray[currentFieldIndex++] = CreateString(line[fieldStartPosition..currentPosition], stringPool);
        ...
        {
            fieldArray[currentFieldIndex] = CreateString(line[fieldStartPosition..], stringPool);
        }
