...
public sealed class SIMDOptimizedParsingStrategy : IParsingStrategy
{
    ...
    public string[] Parse(ReadOnlySpan<char> line, CsvOptions options)
    {
        ...
        {
            var delimiterIndex = line[startIndex..].IndexOfAny(_delimiterSearch);

            if (delimiterIndex < 0)
            {
                // Last field
                var lastFieldSpan = line[startIndex..];
    ...
    private int CountFields(ReadOnlySpan<char> line)
    {
        ...
        {
            var nextDelimiter = line[index..].IndexOfAny(_delimiterSearch);
            if (nextDelimiter < 0)
                break;
