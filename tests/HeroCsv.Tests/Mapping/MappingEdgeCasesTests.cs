...
public class MappingEdgeCasesTests
{
    ...
    {
        public int Id { get; set; }
        public List<string> StringList { get; set; } = []; // Not supported
        public Dictionary<string, int> Dictionary { get; set; } = []; // Not supported
...
3,30.5,2023/01/15,1";
        ...
        // This will use default values for invalid conversions (MapRecord catches exceptions)
        var results = Csv.Read<PropertyTypeMismatchModel>(csv).ToList();
        Assert.Equal(3, results.Count);
        // Second record will have default values for invalid fields
        Assert.Equal(2, results[1].Id);
        Assert.Equal(0, results[1].Age); // Default int
        Assert.Equal(default(DateTime), results[1].Date); // Default DateTime
        Assert.False(results[1].Active); // Default bool
    }
