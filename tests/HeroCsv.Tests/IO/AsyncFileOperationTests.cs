...
public class AsyncFileOperationTests
{
    ...
    [Fact]
    public async Task ReadStreamAsync_WithStream_ProducesCorrectResults()
    {
        // Arrange
        var csvBytes = Encoding.UTF8.GetBytes(TestCsvWithHeader);
        await using var stream = new MemoryStream(csvBytes);
        ...
        // Act
        var records = await Csv.ReadStreamAsync(stream, options, cancellationToken: TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task ReadFileAsync_ProducesCorrectResults()
    {
        // Arrange
        var tempFile = Path.GetTempFileName();
        await File.WriteAllTextAsync(tempFile, TestCsvWithHeader, TestContext.Current.CancellationToken);
        ...
        {
            // Act
            var records = await Csv.ReadFileAsync(tempFile, options, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ReadRecordsAsync_WithCancellationToken_RespectsCancellation()
    {
        ...
        try
        {
            ...
            try
            {
                await foreach (var record in Csv.ReadFileAsyncEnumerable(tempFile, options, cancellationToken: cts.Token))
                {
                    recordCount++;
                    if (recordCount == 5)
                    {
            ...
            {
                // Expected - should have read 5 records before cancellation
    ...
    public async Task ReadRecordsAsync_WithAsyncStreamEnumeration_WorksCorrectly()
    {
        ...
        // Assert - ReadStreamAsync returns all records
        // No need for manual enumeration
        Assert.Equal(3, records.Count);
        Assert.Equal("John", records[0][0]);
        Assert.Equal("Jane", records[1][0]);
        Assert.Equal("Bob", records[2][0]);
    ...
    [Fact]
    public async Task ReadStreamAsync_WithAsyncEnumerable_ProducesCorrectResults()
    {
        // Arrange  
        var csvBytes = Encoding.UTF8.GetBytes(TestCsvWithHeader);
        await using var stream = new MemoryStream(csvBytes);
        ...
        // Note: We don't have a ReadStreamAsyncEnumerable method, so we use file-based approach
        // or implement sync-over-async for testing
        var allRecords = await Csv.ReadStreamAsync(stream, options, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AsyncReader_WithStreamLeaveOpen_DoesNotDisposeStream()
    {
        ...
        // Act
        await Csv.ReadStreamAsync(stream, options, leaveOpen: true, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task AsyncReader_WithStreamDispose_DisposesStream()
    {
        ...
        // Act
        await Csv.ReadStreamAsync(stream, options, leaveOpen: false, cancellationToken: TestContext.Current.CancellationToken);
