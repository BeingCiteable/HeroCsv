...
public class HeaderHandlingTests
{
    ...
    public async Task AsyncMethods_WithHeader_SkipHeaderCorrectly()
    {
        ...
        try
        {
            ...
            var records4 = await Csv.ReadFileAsync(tempFile, options, cancellationToken: TestContext.Current.CancellationToken);
            ...
            await foreach (var record in Csv.ReadFileAsyncEnumerable(tempFile, options, cancellationToken: TestContext.Current.CancellationToken))
            {
                records5.Add(record);
            }
