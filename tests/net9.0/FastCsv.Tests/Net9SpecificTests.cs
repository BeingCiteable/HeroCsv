...
public class Net9SpecificTests
{
    [Fact]
    ...
    [Fact]
    public void Vector512SupportAvailable()
    {
        // Test that Vector512 is available in NET9+
        var isSupported = Vector512.IsHardwareAccelerated;

        // This may be false on systems without AVX-512 support, but the type should be available
        Assert.True(isSupported || !isSupported); // Just verify no exceptions
    }

    [Fact]
    public void FrozenCollectionsAvailable()
    {
        // Test that FrozenSet/FrozenDictionary are available in NET9+
