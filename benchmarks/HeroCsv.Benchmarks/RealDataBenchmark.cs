...
public class RealDataBenchmark
{
    ...
    {
        AnsiConsole.Clear();
        AnsiConsole.Write(new Rule("[yellow]HeroCsv Real Data Performance Analysis[/]").RuleStyle(Style.Parse("yellow")));
        AnsiConsole.WriteLine();

        var resultSet = new BenchmarkResultSet
        {
            BenchmarkSuite = "HeroCsv Real Data Performance",
    ...
    private static void BenchmarkFile(string filePath, string fileName, string description, BenchmarkResultSet resultSet)
    {
        ...
        try
        {
            ...
            var syncResult = BenchmarkActionWithResult(() =>
            {
                ...
                var records = global::HeroCsv.Csv.ReadAllRecords(content, options);
                recordCount = records.Count;
                return recordCount;
            }, iterations, "Sync ReadAllRecords", description, "HeroCsv", recordCount, fileSize);
            ...
            {
                var content = File.ReadAllText(filePath);
                return global::HeroCsv.Csv.CountRecords(content, options);
            }, iterations, "Count Only", description, "HeroCsv", recordCount, fileSize);
            ...
            var streamResult = BenchmarkActionWithResult(() =>
            {
                ...
                using var reader = global::HeroCsv.Csv.CreateReader(stream, options);
                return reader.CountRecords();
            }, iterations, "Stream Reading", description, "HeroCsv", recordCount, fileSize);
            ...
            {
                var records = await global::HeroCsv.Csv.ReadFileAsync(filePath, options, null, CancellationToken.None);
                return records.Count;
            }, Math.Min(iterations, 10), "Async ReadFileAsync", description, "HeroCsv", recordCount, fileSize);
            ...
            var asyncStreamResult = BenchmarkAsyncActionWithResult(async () =>
            {
                ...
                var records = await global::HeroCsv.Csv.ReadStreamAsync(stream, options, null, false, CancellationToken.None);
                return records.Count;
            }, Math.Min(iterations, 10), "Async Stream", description, "HeroCsv", recordCount, fileSize);
    ...
    private static void BenchmarkLargeFile(string filePath, BenchmarkResultSet resultSet)
    {
        ...
        {
            var count = 0;
            await foreach (var record in global::HeroCsv.Csv.ReadFileAsyncEnumerable(filePath, options, null, CancellationToken.None))
            {
                count++;
            }
            return count;
